...
def animate_pixels(imfile1,imfile2,filename,verbose=False):
...
def animate_pixels_color(imfile1,imfile2,filename,verbose=False):
    ...
    if verbose: bar1 = IncrementalBar("Sorting", max=2,suffix='%(percent)d%%')
    rows1,cols1,colors1 = color_to_coords(img1)
    if verbose: bar1.next()
    rows2,cols2,colors2 = color_to_coords(img2)
    if verbose: bar1.next(); bar1.finish()
